	...
	endRun() {// eslint-disable-line complexity
		...
		if (this.selectionInsights.selectionCount === 0) {
			if (this.selectionInsights.testFileCount === 0) {
				...
				this.lineWriter.writeLine(colors.error(`${figures.cross} Based on your configuration, ${count} test ${plur('file was', 'files were', count)} found, but did not match your selection:` + firstLinePostfix));
				this.lineWriter.writeLine();
